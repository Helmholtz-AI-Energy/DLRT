...
class Conv2d_lr(torch.nn.Module):
    def __init__(
        ...
        self.weight = torch.nn.Parameter(
            torch.empty(
                tuple([self.out_channels, self.in_channels] + self.kernel_size),
                **factory_kwargs,
            ),
            requires_grad=True,
        )
        ...
        if self.lr and not self.full_rank_construct:
            if not self.fixed:
                ...
                _, s_ordered, _ = torch.linalg.svd(
                    torch.diag(torch.abs(torch.randn(2 * self.rmax))),
                )
                ...
                self.U_hat = torch.nn.Parameter(
                    torch.randn(n, 2 * self.rmax).to(device),
                    requires_grad=False,
                )
                self.V_hat = torch.nn.Parameter(
                    torch.randn(m, 2 * self.rmax).to(device),
                    requires_grad=False,
                )
                ...
                self.N_hat = torch.nn.Parameter(
                    torch.randn(2 * self.rmax, self.rmax).to(device),
                    requires_grad=False,
                )
                ...
                self.S = torch.nn.Parameter(
                    torch.randn(self.rmax, self.rmax).to(device),
                    requires_grad=False,
                )
                self.weight = None
                self.id = id(self.K)
                ...
                self.N_hat = torch.nn.Parameter(
                    torch.randn(self.rank, self.rank).to(device),
                    requires_grad=False,
                )
                self.M_hat = torch.nn.Parameter(
                    torch.randn(self.rank, self.rank).to(device),
    ...
    def switch_lowrank(self):
        ...
        if self.lr:
            if not self.fixed:
                ...
                r = len(S_load)
                _, s_ordered, _ = torch.linalg.svd(
                    torch.diag(torch.abs(torch.randn(2 * self.rmax - r))),
                )
    ...
    def forward(self, input):
        ...
        if not self.lr:
            ...
            if self.step == "K":
                ...
                if not self.fixed:
                    U_hat, S_hat, V_hat = (
                        self.U_hat[:, : 2 * self.dynamic_rank],
                        self.S_hat[
                            : 2 * self.dynamic_rank,
                            : 2 * self.dynamic_rank,
                        ],
