...
class DLRALayer(keras.layers.Layer):
    ...
    def build_model(self):
        ...
        self.aux_b = self.add_weight(
            shape=(self.units,), initializer="random_normal", trainable=False, name="aux_b"
        )
...
class DLRALayerAdaptive(keras.layers.Layer):
    ...
    def l_step_postprocessing_adapt(self):
        ...
        aux_M = tf.matmul(
            self.aux_Vtnp1[: 2 * self.low_rank, :], tf.transpose(self.aux_Vt[: self.low_rank, :])
        )
    ...
    def s_step_preprocessing(self):
        s = tf.matmul(
            tf.matmul(
                self.aux_N[: 2 * self.low_rank, : self.low_rank], self.s[: self.low_rank, : self.low_rank]
            ),
