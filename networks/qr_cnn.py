...
from compare_basis import CompareQR, QRProjectWeights

from projecting import ProjectWeightsQR, ProjectWeightsHoldQ
...
def main(config):  # noqa: C901
    ...
    if config["dataset"] == "imagenet":
        ...
        dset_dict = dsets.get_cifar100_datasets(
            config['data_location'], config['local_batch_size'], config['workers']
        )
    else:
    ...
    # project_qr = QRProjectWeights(delay=2, skip_first_layer=True)
    ...
    for epoch in range(config['start_epoch'], config['epochs']):
        ...
        # if epoch < config["qr_warmup"] and dist.is_initialized():
        # with model.no_sync():
        train_loss = train(
            train_loader, optimizer, ddpmodel, criterion, epoch, device, config,
            warmup_scheduler=warmup_scheduler, projector=projector, lr_schedule=scheduler
        )
        # else:
        # # with model.no_sync():
        # train_loss = train(
        ...
        # TODO: add this again after baseline is done
        ...
        with warmup_scheduler.dampening():
            ...
            elif config["lr_schedule"]["name"] in ['CosineAnnealingWarmRestarts',
                                                   'CosineAnnealingLR']:
                pass
...
def train(
        train_loader, optimizer, model, criterion, epoch, device, config, warmup_scheduler,
        projector: ProjectWeightsQR, lr_schedule
):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        # else:
        #     projector.project_weights(sync_level='r')

        # if i == 3:
        #     raise ValueError
        # if epoch >= config["qr_warmup"] and (i == len(train_loader) - 1 or i % 10 == 0):
